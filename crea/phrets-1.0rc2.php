...
class phRETS {
   ...
   */
	...
	public function FirewallTest() {
		...
		$crea80 = $this->FirewallTestConn("sample.data.crea.ca", 80);
		...
		if (!$google && !$crea80 && !$crt80 && !$crt6103 && !$flexmls80 && !$flexmls6103) {
		...
		if ($google && $crt6103 && $crea80 && $crt80 && $flexmls6103 && $flexmls80) {
		...
		if (!$google || !$crea80 || !$crt80 || !$flexmls80) {
	...
	public function SearchQuery($resource, $class, $query = "", $optional_params = array()) {
		...
		$search_arguments['Limit'] = empty($optional_params['Limit']) ? 'None' : $optional_params['Limit'];
